...
est.final <- foreach(rep=1:J, .combine = rbind, .packages = c("mice","MASS")) %dopar% {
  ...
  res.df <- data.frame(method=rep(c("SIO1","SIO2"),each=3),estimand=c("NIE","NDE","ATE"),rep=rep,
                       est=c(nie.hat.sio1,nde.hat.sio1,ate.hat.sio1,nie.hat.sio2,nde.hat.sio2,ate.hat.sio2))
  res.df
...
save(est.final,file = "SIO_bs.rdata")
