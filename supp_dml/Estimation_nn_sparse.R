...
train_weighted_regressor <- function(
    label, model_fn, p_in,
    X_train_tensor, y_train_tensor, w_tensor, X_test_tensor,
    epochs, batch_size, lr = 1e-3,
    transform = c("identity", "softplus", "softplus_plus1", "sigmoid01"),
    eps = 1e-6,
    p_floor = 1e-6
) {
  transform <- match.arg(transform)
  apply_transform <- function(t) {
    if (transform == "identity") {
      t
    } else if (transform == "softplus") {
      nnf_softplus(t)                         # (0,+∞)
    } else if (transform == "softplus_plus1") {
      1 + nnf_softplus(t)                     # (1,+∞)
    } else if (transform == "sigmoid01") {
      torch_sigmoid(t) * (1 - 2*p_floor) + p_floor      # (p_floor, 1 - p_floor)
    } else {
      t
    }
  }
  ...
  model$train()
  nrows <- X_train_tensor$size()[[1]]
  for (epoch in 1:epochs) {
    epoch_loss <- 0
    num_batches <- max(1, nrows %/% batch_size)
    idx <- sample(nrows)
    for (b in 1:num_batches) {
      ...
      opt$zero_grad()
      pred_raw <- model(X_b)
      pred     <- apply_transform(pred_raw)
      loss     <- torch_mean(w_b * (y_b - pred)$pow(2))
      loss$backward()
      opt$step()
    ...
    if (epoch %% 10 == 0 || epoch == epochs) {
      cat(sprintf("%s (xfm=%s), Epoch %3d/%3d, Loss: %.6f\n",
                  label, transform, epoch, epochs, epoch_loss / num_batches))
    }
  }
  
  model$eval()
  with_no_grad({
    train_pred <- apply_transform(model(X_train_tensor)$detach())
    test_pred  <- apply_transform(model(X_test_tensor)$detach())
  })
  
  list(
    model        = model,
    train_tensor = train_pred,
    test_tensor  = test_pred,
    train        = as_array(train_pred),
    test         = as_array(test_pred)
  )
}
...
estimate_gamma_eta <- function(MX_tr_t, MX_te_t, X_tr_t, X_te_t,
                               Y_tr_t, TT_tr_t, Rb_tr_t,
                               deltahat_tr_t,
                               pmx, px,
                               epochs, batch_size) {
  w_gamma <- TT_tr_t * Rb_tr_t * deltahat_tr_t
  w_eta   <- (1 - TT_tr_t) * Rb_tr_t * deltahat_tr_t
  
  # gamma: (M,X) -> Y
  res_g <- train_weighted_regressor("gamma", gamma_model_fn, pmx,
                                    MX_tr_t, Y_tr_t, w_gamma, MX_te_t,
                                    epochs, batch_size, lr = 1e-3)
  # eta: X -> gamma_hat
  res_e <- train_weighted_regressor("eta",   eta_model_fn,   px,
                                    X_tr_t, res_g$train_tensor, w_eta, X_te_t,
                                    epochs, batch_size, lr = 1e-3)
  list(
    gamma_train = res_g$train_tensor, gamma_test = res_g$test_tensor,  gamma_test_vec = res_g$test,
    eta_train   = res_e$train_tensor, eta_test   = res_e$test_tensor,  eta_test_vec   = res_e$test
  )
}

estimate_b1_b2 <- function(X_tr_t, X_te_t, MX_tr_t, MX_te_t,
                           TT_tr_t, Rb_tr_t, deltahat_tr_t,
                           px, pmx, epochs, batch_size, eps = 1e-6, p_floor = 0.01) {
  # weights
  w_rb <- Rb_tr_t
  
  # ---------- b1 ----------
  # Step 1: numerator > 1 via 1 + softplus
  t_b1_num <- deltahat_tr_t
  b1_num_res <- train_weighted_regressor("b1_num", b1_model_fn, px,
                                         X_tr_t, t_b1_num, w_rb, X_te_t,
                                         epochs, batch_size, lr = 1e-3,
                                         transform = "softplus_plus1", eps = eps)
  b1_num_train <- b1_num_res$train_tensor
  b1_num_test  <- b1_num_res$test_tensor
  
  # Step 2: denominator is trained on (1 - TT)*delta / numerator and mapped to (0,1) via sigmoid
  t_b1_den_reg <- ((1 - TT_tr_t) * deltahat_tr_t) / (b1_num_train + eps)

  b1_den_res <- train_weighted_regressor("b1_den", b1_model_fn, px,
                                         X_tr_t, t_b1_den_reg, w_rb, X_te_t,
                                         epochs, batch_size, lr = 1e-3,
                                         transform = "sigmoid01", eps = eps, p_floor = p_floor)
  b1_den_train <- b1_den_res$train_tensor
  b1_den_test  <- b1_den_res$test_tensor
  
  # Final b1 estimate = 1 / denom \in (1, \infty)
  b1_train_t <- 1 / (b1_den_train + eps)
  b1_test    <- 1 / (b1_den_test  + eps)
  
  b1_train_t <- winsorize_tensor(b1_train_t, upper_q = 0.99)
  b1_test <- winsorize_tensor(b1_test, upper_q = 0.99)
  
  
  # ---------- b2 ----------
  # Targets as before
  t_b2_num <- (1 - TT_tr_t) * deltahat_tr_t
  t_b2_den <- TT_tr_t * deltahat_tr_t
  # Enforce positivity on both num/den via softplus
  b2_num <- train_weighted_regressor("b2_num", b2_model_fn, pmx,
                                     MX_tr_t, t_b2_num, w_rb, MX_te_t,
                                     epochs, batch_size, lr = 1e-3,
                                     transform = "softplus", eps = eps)
  b2_den <- train_weighted_regressor("b2_den", b2_model_fn, pmx,
                                     MX_tr_t, t_b2_den, w_rb, MX_te_t,
                                     epochs, batch_size, lr = 1e-3,
                                     transform = "softplus", eps = eps)
  b2_num_train <- b2_num$train_tensor
  b2_den_train <- b2_den$train_tensor
  b2_num_test  <- b2_num$test_tensor
  b2_den_test  <- b2_den$test_tensor
  
  b2_train_t <- (b2_num_train * b1_train_t) / (b2_den_train + eps)
  b2_test    <- (b2_num_test  * b1_test)    / (b2_den_test  + eps)

  b2_train_t <- winsorize_tensor(b2_train_t, lower_q = 0.01, upper_q = 0.9)
  b2_test <- winsorize_tensor(b2_test, lower_q = 0.01, upper_q = 0.9)
  
  
  list(
    b1_train_t = b1_train_t,
    b1_test    = as.numeric(b1_test),
    b2_train_t = b2_train_t,
    b2_test    = as.numeric(b2_test),
    # debug/aux
    b1_num_train = b1_num_train,
    b1_den_train = b1_den_train,
    b1_num_test  = as.numeric(b1_num_test),
    b1_den_test  = as.numeric(b1_den_test),
    b2_num_train = b2_num_train,
    b2_den_train = b2_den_train,
    b2_num_test  = as.numeric(b2_num_test),
    b2_den_test  = as.numeric(b2_den_test)
  )
}
...
est_dml <- function(data.input,
                    ...
                    K_fold = 5,
                    eps = 1e-6,
                    p_floor = 0.05 # truncation for propensity score
) {
  ...
  b1_num_hat <- b1_den_hat <- b2_num_hat <- b2_den_hat <- numeric(n)
  ...
  for (fold in 1:K_fold) {
    ...
    # features
    fX   <- cbind(X, TT, M_norm, Y_norm) #pr(R=1|fX)
    # fX   <- cbind(X)
    ...
    # ---------- gamma & eta ----------
    gres <- estimate_gamma_eta(MX_tr_t, MX_te_t, X_tr_t, X_te_t,
                               Y_tr_t, TT_tr_t, Rb_tr_t, deltahat_tr_t,
                               pmx, px, epochs, batch_size)
    gamma_tr_t <- gres$gamma_train
    eta_tr_t   <- gres$eta_train
    gamma_hat[test_idx] <- gres$gamma_test_vec
    eta_hat[test_idx]   <- gres$eta_test_vec
    
    # ---------- b1 & b2 ----------
    bres <- estimate_b1_b2(X_tr_t, X_te_t, MX_tr_t, MX_te_t,
                           TT_tr_t, Rb_tr_t, deltahat_tr_t,
                           px, pmx, epochs, batch_size, eps, p_floor)
    b1_tr_t <- bres$b1_train_t; b2_tr_t <- bres$b2_train_t
    b1_hat[test_idx] <- bres$b1_test
    b2_hat[test_idx] <- bres$b2_test
    
    #temporary evaluation
    b1_num_hat[test_idx] <- bres$b1_num_test
    b1_den_hat[test_idx] <- bres$b1_den_test
    b2_num_hat[test_idx] <- bres$b2_num_test
    b2_den_hat[test_idx] <- bres$b2_den_test
