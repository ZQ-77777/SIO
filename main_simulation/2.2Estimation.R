...
#   - MEMC_DGP2(): implements the proposed SIO estimator.
...
# ------------------------------------------------------------------------------
# MEMC_DGP2(): Implements the proposed SIO estimator (and, optionally, variance estimation).
#
# Inputs:
...
# ------------------------------------------------------------------------------
MEMC_DGP2 <- function(data.input,var.cal=F,...){
  ...
  Y_norm = normalize(Y)
  M_norm = normalize(M)
  Z <- cbind(M_norm,Y_norm)
  X_norm = X
  rho <- function(x){1+exp(-x)}
  
  
  Bas.int.mat.all = Rb*polynomial_basis1(X_norm)
  Jn.all=ncol(Bas.int.mat.all)
  
  ####### r = (1,0) ########
  Bas.ext.mat.all = R1*polynomial_basis4(Z,cbind(X_norm[,c(1,3)]))
  ...
  ####### r = (0,1) ########
  Bas.ext.mat.all = R2*polynomial_basis4(Z,cbind(X_norm[,c(2,3)]))
  kn.all = ncol(Bas.ext.mat.all)
  ...
  ####### r = (0,0) ########
  R0 <- (R1==R2)
  Bas.ext.mat.all = R0*polynomial_basis1(cbind(Z,X_norm[,3]))
  kn.all = ncol(Bas.ext.mat.all)
  ...
  if (var.cal==T){
    ...
    ####### r = (1,0) ########
    Bas.ext.mat = R1*polynomial_basis4(Z,cbind(X_norm[,c(1,3)]))
    ...
    ####### r = (0,1) ########
    Bas.ext.mat = R2*polynomial_basis4(Z,cbind(X_norm[,c(2,3)]))
    ...
    ####### r = (0,0) ########
    R0 <- (R1==R2)
    Bas.ext.mat = R0*polynomial_basis1(cbind(Z,X_norm[,3]))
...
MI <- function(data.input,bs.rep=50){
  ...
  X[R2==0,2] <- NA
  data.imp <- data.input
  imp <- mice(cbind(TT,X),method="norm.predict",maxit=2,m=2,seed=1,printFlag=F)
  data.imp$X <- as.matrix(complete(imp)[,-1])
